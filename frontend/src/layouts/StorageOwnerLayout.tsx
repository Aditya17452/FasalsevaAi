...
const ITEMS = [
  ...
  { to: "/storage/my-storage", label: "My Storage", icon: Warehouse },
  { to: "/storage/bookings", label: "Bookings", icon: CalendarClock },
  { to: "/storage/availability", label: "Availability", icon: Store },
  { to: "/storage/pricing", label: "Pricing", icon: Ticket },
  { to: "/storage/analytics", label: "Analytics", icon: BarChart3 },
  { to: "/storage/profile", label: "Profile", icon: User },
  { to: "/storage/settings", label: "Settings", icon: Settings },
];
...
export function StorageOwnerLayout() {
  ...
  const SidebarInner = (
    <div className="flex h-full flex-col">
      <Link to="/storage/dashboard" className="flex items-center gap-2 px-5 py-5">
        <span className="grid h-9 w-9 place-items-center rounded-xl gradient-primary">
          <Leaf className="h-5 w-5 text-primary-foreground" />
        </span>
        <span className="text-lg font-bold">Storage <span className="text-xs text-muted-foreground">Portal</span></span>
      </Link>
        ...
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between gap-3 border-b border-border/60 bg-background/80 px-4 backdrop-blur sm:px-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2 rounded-full border border-border/60 bg-card px-2 py-1">
              ...
              <span className="pr-2 text-sm font-medium">{user?.name ?? "Storage Owner"}</span>
            </div>
          </div>
        </header>
